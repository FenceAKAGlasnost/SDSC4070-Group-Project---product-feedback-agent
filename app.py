...
from agents import create_crew   # <-- your real multi‑agent pipeline
    ...
    if st.button("Analyze Feedback", type="primary", use_container_width=True):
        ...
        if not final_comments or len(final_comments.strip()) < 30:
            ...
            with st.spinner("Analyzing feedback..."):
                # -----------------------------------------------------------------
                # MODE 1: REAL MULTI‑AGENT PIPELINE (CrewAI)
                # -----------------------------------------------------------------
                if agent_mode == "1. Full 5-Agent Analysis (Recommended)":
                    try:
                        crew = create_crew(final_comments)
                        result = crew.kickoff()          # returns markdown report
                        st.markdown(result)
                        st.success("Multi‑Agent Analysis Complete!")

                        # Save a simplified history entry (you can later parse the report)
                        save_to_history(
...
Output the full report in markdown format with headings."""
                    ...
                    # Save simplified history for these modes
                    if agent_mode == "2. Quick Summary Agent":
...
with tab2:
    ...
    if 'history' in st.session_state and st.session_state.history:
        ...
        for entry in reversed(st.session_state.history):
            ...
            themes = entry.get('key_themes', [])
            if isinstance(themes, list):
                themes_preview = ", ".join(str(t)[:50] for t in themes[:2])
            else:
